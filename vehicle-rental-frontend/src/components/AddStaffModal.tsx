...
import { X, UserPlus, Mail, Briefcase, Lock, Loader2 } from 'lucide-react';
...
export default function AddStaffModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(error.response?.data?.error || error.response?.data?.message || "Failed to create staff.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              ...
              className="flex-1 py-4 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-900/30 transition disabled:opacity-50"
            >
