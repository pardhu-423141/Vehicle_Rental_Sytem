...
export default function AddVehicleModal({ isOpen, onClose, onSuccess, initialData }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    transmission: 'MANUAL', rentalRate: '', imageUrl: '',
    status: 'Available', type: 'FIVE_SEATER', managerId: ''
  });
  ...
  useEffect(() => {
    if (initialData && isOpen) {
      setFormData({
        ...
        licensePlate: initialData.plate || initialData.licensePlate || '',
        color: initialData.color || 'White',
        fuelType: initialData.fuelType || 'PETROL',
        transmission: initialData.transmission || 'MANUAL',
        rentalRate: initialData.price?.toString() || initialData.rentalRate?.toString() || '',
  ...
  useEffect(() => {
    const fetchManagers = async () => {
      try {
        ...
        const vehicleManagers = data.filter((m: any) => m.role === 'VEHICLE_MANAGER');
        setManagers(vehicleManagers);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && initialData) {
        await api.put(`/admin/vehicles/${initialData.id}`, payload);
      ...
      <div className="relative w-full max-w-3xl bg-slate-900 border border-white/20 rounded-3xl shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div>
            ...
            <p className="text-xs text-blue-400 mt-1 uppercase tracking-widest font-semibold">Asset Configuration</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            <div className="space-y-4">
              ...
              <input required placeholder="License Plate"
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <input required type="number" placeholder="Rental Rate / Day"
          ...
          <div className="flex gap-4 pt-6">
            <button type="button" onClick={onClose}
              className="flex-1 py-4 bg-white/5 text-white font-bold rounded-xl border border-white/10 hover:bg-white/10 transition">
              Cancel
            </button>
